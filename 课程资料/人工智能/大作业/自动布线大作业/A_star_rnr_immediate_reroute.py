...
#   整体流程：
#     1. 线网按曼哈顿距离排序，短线优先入队
#     2. 逐条出队 → 标准 A* → 幽灵 A*（罚金递减 600→300→0）
...
#
#   寻路函数：
#     - find_single_path: 标准 A*，曼哈顿启发，四连通
    ...
    while queue:
        ...
        while reroute_stack:
            ...
            for nid in new_conflicts:
                ...
                ripup_count[pair] = ripup_count.get(pair, 0) + 1
                if ripup_count[pair] <= MAX_RIPUPS:
                    reroute_stack.append((nid, victim_id)) #被拆线路入栈，后进先出
