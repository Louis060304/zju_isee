...
#     对每条被淘汰线网：
#       1. 幽灵寻路（罚金 600）建立候选路径
#       2. 拆除挡路的已布线网，立即用标准 A* 全部重布（必须无重叠）
#       3. 全部重布成功 → 提交；任一失败 → 回滚，该线网彻底淘汰
#
#   关键参数：
#     PHASE1_PENALTY        = 300   # Phase 1 固定罚金
#     RESURRECTION_PENALTIES = [300, 600, 1200]  # Phase 2 阶梯罚金
#     PHASE3_PENALTY        = 600   # Phase 3 固定罚金
#     MAX_RESURRECTIONS     = 3     # 每条线网复活次数上限
#
#   寻路函数：
#     - find_single_path: 标准 A*，曼哈顿启发，四连通，不允许重叠
#     - find_ghost_path:  带代价 A*，soft_penalty 字典逐格指定穿越罚金
#
# ═══════════════════════════════════════════════════════════════════════════════
...
def solve(case: Case) -> dict[int, list]:
    ...
    三阶段算法：
      Phase 1：所有线网同时幽灵布线，罚金衰减 max(50, 300-50*n)，允许重叠，不拆线。
      Phase 2：迭代替换最差线网，罚金阶梯递增 300→600→1200，3 次复活后永久淘汰。
      Phase 3：对被淘汰线网尝试事务型救援——幽灵通路线路通才能最终落地。
    """
