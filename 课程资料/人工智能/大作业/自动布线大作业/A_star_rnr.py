...
"""
#
# 布线逻辑：RNR (Rip-up & Reroute) 幽灵模式 — 队列调度 + 复活机制
#
#   整体流程：
#     1. 线网按曼哈顿距离从小到大排序，短线优先入队
#     2. 逐条出队 → 标准 A*（干净路径，不破坏任何已有线网）
#     3. 标准 A* 失败 → 幽灵 A*（允许穿越已有线网，代价由罚金控制）
#     4. 幽灵 A* 失败 → 消耗复活次数（共 2 次），排到队尾等待重试
#     5. 幽灵 A* 成功 → 拆除被踩线网，更新冲突历史地价，被拆线网排回队尾
#
#   三条保护线：
...
#
#   寻路函数：
#     - find_single_path: 标准 A*，曼哈顿启发，四连通，occupied_set 绝对不可通行
#     - find_ghost_path:  带代价 A*，硬障碍不可通行，软障碍 step_cost += penalty + 5^conflict
#
# ═══════════════════════════════════════════════════════════════════════════════
...
def find_single_path(
    ...
    # 优先队列：(f_score, counter, row, col)，counter 用于打破平局
    counter = 0
...
def find_ghost_path(
    ...
    """带代价惩罚的 A* 寻路。硬障碍不可通行，软障碍需付出额外代价。
    step_cost = 1 + penalty（若在软障碍中）+ 5^conflict_count[cell]。"""
    import heapq
    ...
    while heap:
        ...
        for dr, dc in dirs:
            ...
            # 步进代价 = 基础 1 + 穿墙罚金 + 指数历史地价 (5^{rip_counts})
            step_cost = 1
            if neighbor in soft_obstacles:
                step_cost += penalty
            step_cost += 5 ** conflict_count.get(neighbor, 0)
...
def solve(case: Case) -> dict[int, list]:
    ...
    # ── 全局冲突计数：记录每个格子被撕毁的次数，历史地价 = 5^{count} ──
    conflict_count: dict[tuple[int, int], int] = {}

    # 按曼哈顿距离从小到大排序：短线优先，减少对后续线网的阻塞
    sorted_nets = sorted(
        case.nets,
    ...
    while queue:
        ...
        # ── 更新冲突计数：冲突格子 rip_counts++，历史地价 = 5^{count} ──
        ...
        for nid in conflict_ids:
            pair = (nid, net.id) # 记录该冲突线网在当前线网上被拆线重试的次数
            ripup_count[pair] = ripup_count.get(pair, 0) + 1
            if ripup_count[pair] > MAX_RIPUPS: #避免无限循环或过度尝试
                continue
            for n in case.nets:  # 在全局线网列表（case.nets）中查找与冲突ID匹配的线网对象
                if n.id == nid: # 找到后将线网对象加入待处理队列尾
                    queue.append(n)
                    break
